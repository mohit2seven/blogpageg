...
def edit(slno):
    if ('user' in session and session['user']== params['admin_user']):
        if request.method== 'POST':
            ...
            date=datetime.now()
            if slno=='0':
                post= Posts(title=box_title,slug=slug,content=content,tag_line=tag_line,img_file=img_file,date=date)
                db.session.add(post)
                db.session.commit()
            else:
...
def Contact():
    if(request.method =='POST'):
        ...
        entry= Contacts(name=name,phone=phone,date=datetime.now(),msg=message,email=email)
